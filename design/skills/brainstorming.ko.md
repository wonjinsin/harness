---
name: brainstorming
description: router 가 `clarify`, `plan`, `resume` 를 방출할 때 사용 — PRD/TRD/task writer 실행 전 harness intake 단계.
---
...
`harness-flow.yaml` 기준 라우팅 (`next` 는 **바로 다음** 하위 노드):
...
- 다음 agent 직접 dispatch — 메인 스레드가 `outcome` 을 읽고 `harness-flow.yaml` 을 따라 진행.
- 유저가 이미 답한 질문 되묻기.
- Router 재호출. 피벗이 새 세션을 정당화하면 이 스킬 종료 — 다음 턴에 router 가 돈다.
- `ROADMAP.md` (Complexity 줄 + brainstorming 행) 와 `STATE.md` (Current Position + Last activity) 외 파일 쓰기.
