...
### Step 6 — `next` 결정 후 emit

`using-harness § Core loop` 단계 3–5 에 따라: 유일 후보는 `executor` (`when:` 없음). `done` → `executor`, `error` → `null`. 최종 JSON emit.
...
- 다른 agent/skill 호출 금지. Executor dispatch 금지 — 메인 스레드가 harness-flow.yaml 따름.
