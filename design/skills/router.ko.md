---
name: router
description: 모든 user 턴 시작 시점, 다른 planning 스킬 실행 전에 사용 — `.planning/` 세션을 부트스트랩 (slug + scaffold) 하거나 기존 세션과 매칭해 resume 처리.
---
...
4. **1개 매칭** → 해당 세션 로드. `outcome: "resume"` 를 emit, `session_id` 지정. Brainstorming 의 Step 0 이 숏서킷으로 `harness-flow.yaml` 의 다음 미완료 phase 로 점프한다.
...
`outcome`, `session_id`, `next` (아래 라우팅 표에서 결정) 를 담은 단일 JSON 객체를 만든다. `harness-flow.yaml` 은 `outcome` 필드로 라우팅한다. **`casual` 인 경우 JSON 을 만들지 않고** 유저에게 일반 텍스트로 응답하고 종료.

| Outcome | Next | Payload |
|---------|------|---------|
| `casual` | — (JSON 없음, 인라인 응답) | — |
| `clarify` | `brainstorming` (Q&A 풀 진행) | `{ request, session_id, route: "clarify" }` |
| `plan` | `brainstorming` (Q&A 스킵, 바로 분류) | `{ request, session_id, route: "plan" }` |
| `resume` | `brainstorming` (Step 0 숏서킷으로 다음 미완료 phase) | `{ request, session_id, route: "resume", resume: true }` |
...
- `next`: Step 5 에서 결정된 downstream node id — 위 outcome 들에서는 항상 `"brainstorming"`
