...
4. **1개 매칭** → 해당 세션 로드. `outcome: "resume"` 를 emit, `session_id` 지정. Brainstorming 의 Step 0 이 숏서킷으로 다음 미완료 phase 로 점프한다.
...
두 필드를 담은 단일 JSON 객체를 만들어 최종 메시지 전체로 emit: `{ outcome, session_id }`. **`casual` 인 경우 JSON 을 만들지 않고** 유저에게 일반 텍스트로 응답하고 종료.

| Outcome | Emission |
|---------|----------|
| `casual` | — (JSON 없음, 인라인 응답) |
...
이 emission 으로부터 메인 스레드가 구성하는 하위 payload (메인 스레드가 `outcome` → `route` 로 rename 하고 `request`/`resume?` 를 추가) 는 `../../harness-contracts/payload-contract.ko.md` 의 "router → brainstorming" 섹션에 명세돼 있다.
...
- `outcome`: `"clarify"`, `"plan"`, `"resume"` 중 하나
...
다음 스킬은 `outcome` 에 따라 결정됨 (전체 payload 계약: `../../harness-contracts/payload-contract.ko.md` § "router → brainstorming"):

- `outcome == "clarify"` 또는 `"plan"` 또는 `"resume"` → **필수 하위 스킬:** harness-flow:brainstorming 사용
  Payload: `{ session_id, request, route: <outcome>, resume?: true }` — brainstorming 이 자체 emission 에 `outcome` 키를 예약하므로, 메인 스레드가 `outcome` → `route` 로 rename 한다.
- `outcome == "casual"` → JSON emit 안 함; 흐름 미진입. 사용자에게 직접 답변하고 종료.
