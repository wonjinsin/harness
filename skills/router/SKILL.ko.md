...
매 유저 턴의 **가장 첫 단계**에서 이 스킬을 트리거한다. 다른 스킬이 `flow.yaml` 을 통해 명시적으로 제어권을 넘긴 경우에만 건너뛴다.
...
4. **1개 매칭** → 해당 세션 로드. `route = plan`, `session_id` 지정, `resume: true`. 하위는 분류를 건너뛰고 다음 미완료 phase 로 바로 간다.
...
구조화된 분류 결과를 내보낸다. `flow.yaml` 이 이를 읽고 라우팅한다.

| Route | 다음 스킬 | Payload |
|-------|-----------|---------|
| `casual` | (END — router 가 인라인 응답) | — |
| `clarify` | `brainstorming` | `{ request, session_id }` |
| `plan` (신규) | `complexity-classifier` | `{ request, session_id }` |
| `plan` (재개) | `complexity-classifier` (이미 분류됐으면 스킵) → 다음 미완료 phase | `{ request, session_id, resume: true }` |
...
최종 router payload 는 JSON 객체 **하나**로 반환한다. 앞뒤에 설명 문장을 붙이지 않는다.

스키마:

- `route`: `"casual"`, `"clarify"`, `"plan"` 중 하나
- `session_id`: `clarify` / `plan` 일 땐 `"YYYY-MM-DD-slug"`, `casual` 일 땐 `null`
- `resume`: Step 1 에서 기존 세션이 매칭됐을 때만 `true`, 그 외 `false`
- `reply`: `casual` 일 때 유저에게 보일 완성된 응답 문자열, 그 외 `null`

### 예시

입력: `hi claude, what can you build?`
