...
- `signals_matched` *(optional)*: `["path:auth/", "keyword:login", ...]` — Risks 와 Affected surfaces 를 결정하는 근거.
- `estimated_files` *(optional)*: classifier 가 넘긴 정수 — Step 2 탐색 폭 calibration 에 사용.

`prd_path` 가 세팅돼 있는데 파일을 못 읽거나 존재하지 않으면 즉시 중단하고 `{"outcome": "error", "reason": "PRD declared in payload but <path> not found", "stage": "step-1"}` 를 emit. 지어내서 진행하지 말 것 — payload 는 메인 스레드와의 계약이다.

## Output

최종 메시지는 항상 `outcome` 태그가 붙은 JSON 객체 하나. 두 가지 outcome 이 가능.
...
`open_questions` 는 "Open questions" 섹션 아래 항목 개수 (없으면 0). 메인 스레드가 task-writer 돌기 전에 유저에게 노출할 수 있음.
...
**File** (`drafted` outcome 에 한함): `.planning/{session_id}/TRD.md`. 같은 경로에 파일이 이미 있으면 error 를 emit — **절대 overwrite 하지 않음**. 재생성은 메인 스레드 몫: 이전 파일을 먼저 지우고 나서 재-dispatch.
...
Target 주도: 다음 우선순위로 주 파일/모듈을 먼저 찾는다 — `brainstorming_output.target` (있으면), PRD 의 주제 (`prd_path` 세팅돼 있으면), 둘 다 없는 최후의 경우엔 `request` 의 첫 명사구 (예: `"auth middleware 를 별도 패키지로 추출하고 싶다"` → `auth middleware`). 그 다음 payload 시그널로 탐색 폭을 결정:

- `estimated_files` ≥ 5 **또는** `scope_hint: multi-system` **또는** `signals_matched` 가 서브시스템을 횡단 → 직접 caller, 형제 모듈, 이 변경이 건드리는 공유 추상화까지 확장.
- 그 외 → target 파일/모듈과 그 직접 의존성 안에서만.

이게 `estimated_files` 가 payload 에 있는 이유다 — 얼마나 바깥으로 걸어나갈지 calibration 하는 용도지 내용을 결정하지 않는다.
...
파일이 이미 있으면 중단하고 `{"outcome": "error", "reason": "TRD.md already exists at <path>", "stage": "step-4"}` emit. 재생성은 메인 스레드 몫 — 이전 파일을 먼저 지우고 재-dispatch.
...
{구체적 실패 모드 + 설계가 어떻게 완화하거나 수용하는지.
 `signals_matched` 가 auth/security/migrations 를 건드리면 항목이 필요하다 —
...
prd-writer 예시 세션 (`2026-04-19-add-2fa-login`) 을 이어받는 payload `{prd_path: ".planning/2026-04-19-add-2fa-login/PRD.md", signals_matched: ["path:auth/", "keyword:login"], estimated_files: 4}` 가 주어진 경우:
...
Created: 2026-04-19
PRD: ../2026-04-19-add-2fa-login/PRD.md
...
- **중간 토큰 재전송** (`path:auth/`): 토큰은 single-use — verify 엔드포인트가 `jti` 를 in-memory LRU (크기 10k, TTL 5분) 에 consumed 로 마킹. 프로세스 재시작 시 유실은 허용 (TTL 이 이미 짧음).
- **Clock skew 로 TOTP 실패** (`keyword:login`): `otplib` 기본 window 는 ±1 step (30초). 문서화만, 변경 없음.
...
참고: 이 예시는 모든 범위에서 **하단** 수준이다 — 단일 모듈 내 변경, Approach 4 bullet, Affected surfaces 4 항목, Risks 2 항목, Open questions 0. `scope_hint: multi-system` 이거나 `estimated_files ≥ 8` 인 세션은 대개 Affected surfaces 와 Interfaces 가 먼저 늘어나고 (서브시스템 횡단이면 노출되는 계약이 더 많다) 그 다음 Risks 가 늘어난다; 다른 섹션은 범위 안에서 확장하지 밖으로 나가지 않는다. 아래 "PRD 없음" 변형은 Section 1 이 기술 동기 문단으로 바뀌고, 설계 근거가 의지할 PRD 가 없기 때문에 Approach 도 대개 더 늘어난다.

## Example 2 — 렌더된 TRD (상류 PRD 없음)

요청 `"auth middleware 를 별도 패키지로 추출해서 admin API 와 공유하고 싶다"` 와 payload `{prd_path: null, signals_matched: ["path:auth/", "keyword:middleware"], estimated_files: 8}` 가 주어진 경우:
...
- **`auth/` / `security/` / `migrations/` 시그널이 매칭됨**: 템플릿 §7 규칙은 변경이 아무리 작아 보여도 적용된다 — 생략이 곧 조용한 실패 모드이므로 판단을 명시적으로 적는다 (항목이 "accepted: 동작 보존 변경" 같은 형태여도 괜찮음).
...
- **작성 후 Open questions 가 2개 초과**: 최종 JSON 의 `open_questions: N` 에 그대로 반영 — 메인 스레드가 task-writer 돌기 전 유저에게 재확인할지 판단. self-escalate 하지 말 것; 범위 판단은 메인 스레드의 몫.
...
- task-writer 를 dispatch 하지 말 것. 메인 스레드가 flow.yaml 을 따른다.
- 탐색 중 버그를 발견해도 소스 코드를 수정하지 말 것. load-bearing 이면 Open questions 에 적는다.
- Tool 예산: Step 2 에서 Read/Grep/Glob 약 25회 총량. 더 필요하면 payload 에 문제가 있는 것 — 중단하고 error outcome emit.
