...
- `signals_matched` *(optional)*: `["path:auth/", "keyword:login", ...]` — task 경계와 Notes 결정의 근거.
- `estimated_files` *(optional)*: classifier 가 넘긴 정수 — 탐색 폭과 task 개수 기댓값 calibration. 대략 매핑: `≤ 2` → 단일 task 가 많음 (구조를 억지로 만들지 말 것); `3–7` → 전형적인 3–8 task 범위; `≥ 8` → wide refactor, Step 2 예산은 Read 로 파고들지 말고 Glob 으로 열거에 쓸 것.

`prd_path` 가 세팅돼 있는데 파일을 못 읽으면 중단하고 `{"outcome": "error", "reason": "PRD declared in payload but <path> not found", "stage": "step-1"}` emit. `trd_path` 도 동일. 지어내서 진행하지 말 것.

`prd_path` · `trd_path` · `brainstorming_output` 이 모두 null 이고 `request` 가 actionable verb 없는 한 문장 (예: "좋네요") 이면 `{"outcome": "error", "reason": "insufficient input to derive tasks", "stage": "step-1"}` emit. 메인 스레드가 오라우팅한 가능성이 높음; recovery 는 메인 스레드 결정.
...
`task_count` 는 emit 된 task 개수. 메인 스레드는 로깅용; executor 는 파일에서 다시 파싱한다.
...
**File** (`drafted` outcome 에 한함): `.planning/{session_id}/TASKS.md`. 파일이 이미 있으면 error 를 emit — **절대 overwrite 하지 않음**. 재생성은 메인 스레드 몫: 기존 파일을 먼저 지우고 재-dispatch.
...
선언된 상류 파일이 누락돼 있으면 step-1 error outcome emit.
...
예산 cap 에 도달했는데 위 세 질문에 답할 수 없으면 중단하고 `{"outcome": "error", "reason": "codebase exploration exhausted budget without resolving change surface", "stage": "step-2"}` emit. 상류 payload 가 덜 명세된 상태일 가능성이 높음 — 상류 writer 재-dispatch 여부는 메인 스레드 결정.
...
**주먹구구 규칙**: 이 harness 대상 세션은 3–8 task 가 건강한 범위. 3 미만이면 쪼개야 할 걸 묶은 것; 8 초과면 subagent 하나가 한 번에 할 수 있는 걸 쪼갠 것. `estimated_files ≤ 2` 세션은 정확히 1개 task 가 정답인 경우가 많음 — 구조를 억지로 만들지 말 것.
...
파일이 이미 있으면 중단하고 `{"outcome": "error", "reason": "TASKS.md already exists at <path>", "stage": "step-5"}` emit. 재생성은 메인 스레드 몫.
...
## Example 1 — 렌더된 TASKS.md (Tier A: PRD · TRD 둘 다 있음)

trd-writer 의 Example 1 (`2026-04-19-add-2fa-login`) 을 이어받는 payload `{prd_path: ".planning/2026-04-19-add-2fa-login/PRD.md", trd_path: ".planning/2026-04-19-add-2fa-login/TRD.md", signals_matched: ["path:auth/", "keyword:login"], estimated_files: 4}`:
...
## Example 2 — 렌더된 TASKS.md (Tier D: PRD · TRD 둘 다 없음)

`request: "getUser 헬퍼를 fetchUser 로 바꿔줘"` 와 payload `{prd_path: null, trd_path: null, estimated_files: 3}`:
...
- **DAG 에 사이클**: emit 하지 말 것. `{"outcome": "error", "reason": "task DAG contains cycle: task-N → task-M → task-N", "stage": "step-5"}` emit. 재분해 여부는 메인 스레드 결정.
- **비영어 request**: Goal · Architecture · Notes 는 유저 언어; Conventions · 필드명 · 파일 경로 · 코드 식별자 · Self-Review 체크리스트 텍스트는 영어 (기계 판독 계약).
- **`request` 만 있고 범위 넓은 리팩터** (`estimated_files ≥ 8`): Step 2 예산이 빠듯함; 호출 지점 열거에 Glob 을 쓰고 각각을 Read 로 이해하는 데 쓰지 말 것. 리팩터가 균일하면 파일 8개가 Files 블록에 들어간 단일 task 가 허용됨.
...
- Executor 를 dispatch 하지 말 것. 메인 스레드가 flow.yaml 을 따른다.
- 탐색 중 버그를 발견해도 소스 코드를 수정하지 말 것. load-bearing 이면 영향 받는 task 의 Notes 에 적고, 아니면 그냥 둔다.
- Tool 예산: Step 2 에서 Read/Grep/Glob 약 20회 총량. 더 필요하면 payload 또는 상류 docs 에 문제가 있는 것 — 중단하고 error outcome emit.
